...
const StatusIndicator: React.FC<{ availability: RouteAvailability }> = ({ availability }) => {
  const styles = {
    available: {
      bg: 'bg-green-100',
      text: 'text-green-800',
  ...
  const currentStatus = styles[availability];
                            ...
                            <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-5">
                              <div className="flex items-start md:items-center gap-4">
                                ...
                                <div className="flex-grow">
                                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3">
                                    <h3 className="text-lg font-bold text-brand-navy mb-2 sm:mb-0">{`Route No: R-${route.number} – ${route.name}`}</h3>
                                    <StatusIndicator availability={route.availability} />
                                  </div>
                                  <div className="flex items-center gap-3 bg-slate-50 border border-slate-200/80 rounded-lg p-3">
                                    <div className="flex items-center gap-2">
                                      <span className="text-xs font-bold text-slate-500 uppercase">Bus No:</span>
                                      <span className="px-2.5 py-1 bg-white border border-slate-200 text-slate-700 rounded-md text-sm font-semibold font-mono shadow-sm">{route.busNumber}</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-slate-600">
                                      ...
                                      <p className="text-xs font-bold uppercase tracking-wide">{route.status}</p>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center justify-end gap-3 w-full md:w-auto pl-12 md:pl-0">
                                <button
                                  ...
                                  className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-colors border ${isExpanded
            ...
            {activeTab === 'schedule' && (
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden min-h-[600px] animate-in fade-in slide-in-from-right-4 duration-500 flex flex-col">
                ...
                <div className="flex-grow bg-slate-100 p-4 flex items-center justify-center">
                  <iframe
                    src="/schedule.pdf"
                    className="w-full h-[800px] rounded-xl shadow-sm border border-slate-200 bg-white"
